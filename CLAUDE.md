...
make daily-report             # Run automated daily report (Hermes one-shot)
...
Data flow: Source Registry (YAML) → Hermes-Agent (external, daily workflow) → Backend API (store/retrieve/dedup) → MCP Server (15 tool functions for Hermes)
...
- **`mcp/`** — FastMCP server exposing 15 tool functions that Hermes calls via MCP protocol
...
R1-01 through R1-09B are done. R1-10 (stable daily report run modes) is done. R1-11 (daily report quality loop) is done. R1-11B (model-tiered daily report pipeline) is done — small model client, candidate preprocess, MCP tool `risk_candidate_preprocess`, all optional and gracefully disabled. R1-11B+ (website, production delivery) are TODO.
...
- 15 `risk_*` tools allowlisted, prompts/resources disabled
