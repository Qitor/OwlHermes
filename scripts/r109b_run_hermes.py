"""R1-09B automated helper-assisted Hermes dry run runner.

Modes:
  --preflight   Verify all prerequisites without running Hermes.
  --run         Run the full automated helper-assisted Hermes dry run.
...
by default — keeping R1-08 data allows testing seen-check and dedup.
"""
...
PROMPT_FILE = REPO_ROOT / "prompts" / "r109_hermes_helper_assisted_dry_run_prompt.md"
RUNS_DIR = REPO_ROOT / "runs" / "r1_09b"
DRYRUN_DB_PATH = REPO_ROOT / ".local" / "risk_observer_dryrun.db"
DRYRUN_DB_URL = f"sqlite:///{DRYRUN_DB_PATH}"

HERMES_TIMEOUT_SECONDS = 900  # 15 minutes (helper fetches may take longer)
...
def check_helper_preview_nofetch() -> bool:
    """Verify helper preview works without network fetch."""
    python = str(REPO_ROOT / ".venv" / "bin" / "python")
    result = _run_cmd(
...
def preflight() -> bool:
    ...
    # 3. Source health
    print("3. Checking source health...")
    if not check_source_health():
        print("   FAIL: Source health check failed")
        return False
    print("   OK")

    # 4. Helper preview (no-fetch)
    print("4. Checking helper preview (no-fetch)...")
    if not check_helper_preview_nofetch():
        print("   FAIL: Helper preview failed")
        return False
    print("   OK")

    # 5. MCP smoke
    print("5. Checking MCP smoke...")
    ...
    if not check_dryrun_db():
        ...
        return False
    print("   OK (keeping existing R1-08 data for seen-check/dedup testing)")

    # 7. Prompt file
    print("7. Checking R1-09B prompt file...")
...
def run_hermes(run_dir: Path) -> bool:
    """Launch Hermes in one-shot mode with the R1-09B prompt."""
    ...
    print("  This may take several minutes as Hermes uses helpers and browses sources.\n")
...
def _write_summary(
    ...
    lines = [
        "# R1-09B Helper-Assisted Run Summary",
        "",
        f"- **Timestamp**: {run_dir.name}",
        f"- **Hermes exit code**: {result.returncode}",
        "- **Prompt file**: prompts/r109_hermes_helper_assisted_dry_run_prompt.md",
        "- **Mode**: Helper-assisted dry run (non-production)",
        "- **DB**: SQLite dry-run DB (kept R1-08 data for seen-check/dedup testing)",
        "- **Output files**:",
        "  - hermes_stdout.log",
        "  - hermes_stderr.log",
        "  - hermes_output.md",
...
def inspect_state(run_dir: Path | None = None) -> None:
    ...
    python = str(REPO_ROOT / ".venv" / "bin" / "python")
    result = _run_cmd(
        [python, "scripts/r109b_inspect_state.py"],
        check=False,
        env={"DATABASE_URL": DRYRUN_DB_URL},
    )
    print(result.stdout)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="R1-09B Helper-assisted Hermes dry run runner",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Run the full helper-assisted Hermes dry run",
    )
    parser.add_argument(
    ...
    if args.run:
        ...
        print("\n=== Running R1-09B Helper-Assisted Hermes Dry Run ===")
        print(f"  Run directory: {run_dir}")
