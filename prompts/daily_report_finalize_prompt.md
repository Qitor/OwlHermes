...
- `risk_source_health_summary` — 查看来源健康状况
- `risk_candidate_preprocess` — 对长文本获取摘要（仅供参考）

## 你不可以做的事

- **不要浏览网页** — 不要使用 web/search/research
- **不要抓取 URL** — 不要调用 `risk_discovery_helper_preview` 且 fetch=true
- **不要选择新来源** — 不要浏览新的信息源
- **不要进行广泛搜索** — 只使用已有的本地状态
- **不要设置 cron** — 不要运行定时任务
- **不要对外发布** — 不要发送飞书/企微/邮件
...
来源注册表包含 `access_status` 字段。在撰写来源扫描摘要时参考：
- `blocked` 来源无法自动化采集，需要在"需跟进"中说明
- `degraded` 来源可能数据不完整，标注数据覆盖度
- `timeout_prone` 来源可能超时未获取，标注是否成功获取
- `manual_only` 来源需要后续人工审查
